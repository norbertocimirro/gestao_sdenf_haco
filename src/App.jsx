...
const calculateDetailedTime = (dateInput) => {
  ...
  return { 
    ...
    m: Math.max(0, m), 
    d: Math.max(0, d),
    display: `${Math.max(0, y)}a ${Math.max(0, m)}m ${Math.max(0, d)}d`
  };
};
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      <header className="bg-white border-b border-slate-200 p-5 flex justify-between items-center sticky top-0 z-10 shadow-sm">
        ...
        <button onClick={onLogout} className="bg-slate-100 p-2.5 rounded-xl text-slate-400 hover:text-red-500 transition-all active:scale-90"><LogOut size={18}/></button>
      </header>
      <main className="flex-1 p-6 max-w-lg mx-auto w-full space-y-6">
        <div className="bg-blue-600 p-8 rounded-[2rem] text-white shadow-xl relative overflow-hidden"><h2 className="text-xl font-black uppercase tracking-tighter relative z-10">Mural</h2><Plane className="absolute -bottom-6 -right-6 text-white/10" size={120}/></div>
        <div className="grid grid-cols-2 gap-4">
          <button onClick={() => setModals({...modals, atestado: true})} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col items-center gap-3 hover:shadow-xl transition-all group active:scale-95 group"><div className="p-3 bg-red-50 text-red-500 rounded-2xl group-hover:bg-red-500 group-hover:text-white transition-all"><ShieldAlert size={24}/></div><span className="font-black text-[9px] uppercase text-slate-700 tracking-widest">Atestado</span></button>
          <button onClick={() => setModals({...modals, permuta: true})} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col items-center gap-3 hover:shadow-xl transition-all group active:scale-95 group"><div className="p-3 bg-indigo-50 text-indigo-500 rounded-2xl group-hover:bg-indigo-500 group-hover:text-white transition-all"><ArrowRightLeft size={24}/></div><span className="font-black text-[9px] uppercase text-slate-700 tracking-widest">Permuta</span></button>
        </div>
        <div className="pt-8"><h3 className="font-black text-slate-800 text-xs uppercase tracking-[0.2em] mb-4 flex justify-between">Meus Registros <button onClick={fetchData} className="p-2 bg-white border rounded-xl"><RefreshCw size={12} className={loading?'animate-spin':''}/></button></h3>
          <div className="space-y-3">
            {[...data.permutas, ...data.atestados].map((item, i) => (
              <div key={i} className="bg-white p-5 rounded-[1.5rem] border border-slate-100 shadow-sm flex justify-between items-center group transition-all">
                <div className="text-xs">
                  <p className="font-black text-slate-800 uppercase text-xs mb-1">{getVal(item,['substituto']) ? `Troca: ${getVal(item,['substituto'])}` : `Afastamento: ${getVal(item,['dias'])}d`}</p>
                  <div className="flex gap-4 font-bold text-slate-400 text-[9px] uppercase tracking-widest">
...
const MainSystem = ({ user, role, onLogout, globalOfficers, refreshGlobal }) => {
  ...
  const refreshData = async (showFeedback = true) => {
    ...
    try {
      ...
      if (showFeedback) alert("SGA-Enf: Sistema Sincronizado!");
    } catch(e) { console.error(e); } finally { setLoading(false); }
  };
  ...
  const sendData = async (action, payload) => {
    ...
    } catch (e) { setIsSaving(false); alert("Erro de conexão."); }
  };
  ...
  const renderContent = () => {
    if (loading) return <div className="p-20 text-center flex flex-col items-center gap-4"><Loader2 className="animate-spin text-blue-600" size={40}/> <p className="font-black text-slate-400 text-xs uppercase tracking-widest">Sincronizando com HACO...</p></div>;
    ...
    switch(activeTab) {
          ...
          <div className="space-y-6 animate-fadeIn font-sans">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="md:col-span-4 bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-xl flex justify-between items-center border border-slate-800 relative overflow-hidden">
                   ...
                   <div className="flex items-center gap-6 relative z-10"><div className="bg-blue-600 p-4 rounded-3xl shadow-lg shadow-blue-500/20"><Activity size={24}/></div><div><h3 className="text-lg font-black uppercase tracking-tighter">Monitorização UPI</h3><p className="text-slate-500 text-[9px] font-black uppercase tracking-widest mt-1">Sincronizado: {data.upi.dataReferencia}</p></div></div>
                ...
                <div className="bg-white p-6 rounded-[2rem] border flex flex-col items-center justify-center group hover:border-red-200 transition-all shadow-sm">
                  <p className="text-[9px] font-black uppercase text-slate-400 mb-1 group-hover:text-red-500">Atestados</p>
                  <h3 className="text-3xl font-black text-slate-800 tracking-tighter">{data.atestados.filter(x=>getVal(x,['status'])==='Pendente').length}</h3>
                </div>
                <div className="bg-white p-6 rounded-[2rem] border flex flex-col items-center justify-center group hover:border-blue-200 transition-all shadow-sm">
                  <p className="text-[9px] font-black uppercase text-slate-400 mb-1 group-hover:text-blue-600">Efetivo</p>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {sorted.map((o, i) => {
                      ...
                      <tr key={i} className="hover:bg-slate-50/50 group transition-all">
                        ...
                        <td className="p-4"><div className="flex flex-wrap gap-1">{expedientes.length > 0 ? expedientes.map((ex, idx) => (<span key={idx} className="bg-blue-50 text-blue-600 text-[7px] font-black uppercase px-2 py-0.5 rounded-md border border-blue-100">{ex.trim()}</span>)) : <span className="text-slate-300">-</span>}</div></td>
                        ...
                        <td className={`p-4 text-center text-[10px] font-bold ${tServico.y >= 7 ? 'text-red-600 bg-red-50 rounded-xl' : 'text-slate-600'}`}>
                           <div className="flex flex-col items-center">
                              <span className="text-[8px] text-slate-400">{formatDate(getVal(o,['ingres']))}</span>
                        ...
                        <td className="p-4 text-right">
                          <div className="flex gap-2 justify-end opacity-0 group-hover:opacity-100 transition-all">
                            ...
                            }} className="p-2 bg-slate-100 text-slate-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all"><Edit3 size={14}/></button>
                            <button onClick={() => { if(window.confirm(`Remover ${getVal(o,['nome'])}?`)) sendData('deleteOfficer', { nome: getVal(o,['nome']) }); }} className="p-2 bg-slate-100 text-slate-600 rounded-xl hover:bg-red-600 hover:text-white transition-all"><Trash2 size={14}/></button>
                          </div>
                        </td>
                      </tr>
                    )})}
      ...
      case 'atestados':
         return (
            <div className="bg-white rounded-[3rem] shadow-sm border p-8 animate-fadeIn font-sans">
               <div className="overflow-x-auto"><table className="w-full text-left font-bold uppercase"><thead className="text-[9px] text-slate-400 tracking-widest border-b uppercase"><tr><th className="p-6">Militar</th><th className="p-6 text-center">Dias</th><th className="p-6">Início</th><th className="p-6">Status</th><th className="p-6 text-right">Ação</th></tr></thead>
                  <tbody className="divide-y divide-slate-100">
                    {data.atestados.map((a, i) => (
                      <tr key={i} className="hover:bg-slate-50 transition-all">
                        <td className="p-6 text-slate-800 text-sm font-black tracking-tighter">{getVal(a,['militar'])}</td>
                        ...
                        <td className="p-6"><span className={`px-4 py-1.5 rounded-full text-[9px] font-black tracking-widest ${getVal(a,['status']) === 'Homologado' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{getVal(a,['status'])}</span></td>
                        <td className="p-6 text-right">{getVal(a,['status']) === 'Pendente' && <button onClick={()=>sendData('updateStatus',{sheet:'Atestados',id:getVal(a,['id']),status:'Homologado'})} className="bg-blue-600 text-white px-4 py-1.5 rounded-xl text-[9px] uppercase tracking-widest">Homologar</button>}</td>
                      </tr>
                    ))}
                    {data.atestados.length === 0 && <tr><td colSpan="5" className="p-10 text-center text-slate-300 font-black text-xs uppercase tracking-widest">Nenhum registo encontrado</td></tr>}
                  </tbody>
                </table></div>
            </div>
         );
  ...
  return (
    <div className="flex h-screen bg-slate-100 text-slate-800 font-sans overflow-hidden">
      <aside className={`${sidebarOpen ? 'w-72' : 'w-24'} bg-slate-950 text-white transition-all duration-500 flex flex-col z-20 shadow-2xl border-r border-white/5`}>
         <div className="p-8 h-24 flex items-center border-b border-white/5">{sidebarOpen && <span className="font-black text-xl uppercase tracking-tighter">SGA-Enf</span>}<button onClick={() => setSidebarOpen(!sidebarOpen)} className="ml-auto p-2.5 hover:bg-white/10 rounded-2xl transition-all"><Menu size={22} className="text-slate-400"/></button></div>
         ...
         <div className="p-8 border-t border-white/5"><button onClick={onLogout} className="flex items-center gap-5 text-slate-600 hover:text-red-400 font-black text-[10px] uppercase tracking-widest w-full p-4.5"><LogOut size={22}/> {sidebarOpen && 'Sair'}</button></div>
      </aside>
