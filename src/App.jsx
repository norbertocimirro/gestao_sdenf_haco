...
const UserDashboard = ({ user, onLogout }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setData({
        atestados: (resData.atestados || []).filter(a => String(getVal(a, ['militar'])).includes(user)).reverse(),
        permutas: (resData.permutas || []).filter(p => String(getVal(p, ['solicitante'])).includes(user)).reverse(),
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <main className="flex-1 p-6 max-w-lg mx-auto w-full space-y-6">
        <div className="bg-blue-600 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden"><h2 className="text-2xl font-black uppercase tracking-tighter relative z-10">Mural</h2><Plane className="absolute -bottom-10 -right-10 text-white/10" size={180}/></div>
        <div className="grid grid-cols-2 gap-4">
          <button onClick={() => setModals({...modals, atestado: true})} className="bg-white p-8 rounded-[2rem] border border-slate-200 flex flex-col items-center gap-3 hover:shadow-xl transition-all active:scale-95 group"><div className="p-3 bg-red-50 text-red-500 rounded-2xl group-hover:bg-red-500 group-hover:text-white transition-all"><ShieldAlert size={32}/></div><span className="font-black text-[10px] uppercase text-slate-700">Atestado</span></button>
          <button onClick={() => setModals({...modals, permuta: true})} className="bg-white p-8 rounded-[2rem] border border-slate-200 flex flex-col items-center gap-3 hover:shadow-xl transition-all active:scale-95 group"><div className="p-3 bg-indigo-50 text-indigo-500 rounded-2xl group-hover:bg-indigo-500 group-hover:text-white transition-all"><ArrowRightLeft size={32}/></div><span className="font-black text-[10px] uppercase text-slate-700">Permuta</span></button>
        </div>
        <div className="pt-8"><h3 className="font-black text-slate-900 text-lg uppercase tracking-tighter mb-4 flex justify-between">Meus Registros <button onClick={fetchData} className="p-2 bg-white border rounded-xl"><RefreshCw size={14} className={loading?'animate-spin':''}/></button></h3>
          <div className="space-y-3">
            {[...data.permutas, ...data.atestados].map((item, i) => (
              <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex justify-between items-center group">
                <div className="text-xs">
                  <p className="font-black text-slate-800 uppercase text-sm mb-2">{getVal(item,['substituto']) ? `Troca com ${getVal(item,['substituto'])}` : `Afastamento: ${getVal(item,['dias'])}d`}</p>
...
const MainSystem = ({ user, role, onLogout, globalOfficers, refreshGlobal }) => {
  ...
  const handleSaveOfficer = (e) => {
    ...
    const payload = {
      ...
      // Converte array para string separada por vírgula para a planilha
  ...
  const renderContent = () => {
    ...
    switch(activeTab) {
          ...
          <div className="space-y-6 animate-fadeIn">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="md:col-span-4 bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl flex justify-between items-center border border-slate-800">
                   ...
                   <div className="flex gap-12 text-center font-black">
                      <div><p className="text-slate-500 text-[10px] uppercase mb-1">Ocupação</p><p className="text-4xl">{data.upi.leitosOcupados}</p></div>
                      <div><p className="text-slate-500 text-[10px] uppercase mb-1">Braden</p><p className="text-4xl text-yellow-500">{data.upi.mediaBraden.toFixed(1)}</p></div>
                      <div><p className="text-slate-500 text-[10px] uppercase mb-1">Fugulin</p><p className="text-4xl text-green-500">{data.upi.mediaFugulin.toFixed(1)}</p></div>
                   </div>
                </div>
                <div className="bg-white p-8 rounded-[2.5rem] border"><p className="text-[10px] font-black uppercase text-slate-400 mb-2">Pendências</p><h3 className="text-4xl font-black text-red-600">{data.atestados.filter(x=>getVal(x,['status'])==='Pendente').length + data.permutas.filter(x=>getVal(x,['status'])==='Pendente').length}</h3></div>
                <div className="bg-white p-8 rounded-[2.5rem] border"><p className="text-[10px] font-black uppercase text-slate-400 mb-2">Efetivo</p><h3 className="text-4xl font-black text-blue-600">{globalOfficers.length}</h3></div>
                <div className="md:col-span-2 row-span-2 border rounded-[2.5rem] bg-white overflow-hidden p-6 text-slate-400 font-bold uppercase text-[10px] flex items-center justify-center tracking-widest">Mural de Indicadores Mensais</div>
            </div>
          </div>
        );
      case 'efetivo':
         ...
         return (
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 p-8 animate-fadeIn">
              <div className="flex justify-between items-center mb-10">
                ...
                <button onClick={() => { setFormOfficer({ expediente: [], servico: '' }); setShowOfficerModal(true); }} className="bg-blue-600 text-white px-8 py-3.5 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest flex items-center gap-2 active:scale-95 shadow-xl shadow-blue-500/20 transition-all"><UserPlus size={18}/> Novo Militar</button>
              </div>
              <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-slate-400 text-[9px] font-black uppercase tracking-widest border-b"><tr><th className="p-4 text-center">Ant.</th><th className="p-4">Posto/Nome</th><th className="p-4">Expediente</th><th className="p-4">Serviço</th><th className="p-4 text-center">Idade</th><th className="p-4 text-center">Serviço</th><th className="p-4 text-right">Ação</th></tr></thead>
                  <tbody className="divide-y divide-slate-100">
                    {sorted.map((o, i) => {
                      ...
                      const expedientes = getVal(o, ['expediente']).split(',').filter(x => x.trim() !== "");
                      ...
                      <tr key={i} className="hover:bg-slate-50 group font-bold">
                        ...
                        <td className="p-4 text-slate-800 uppercase tracking-tighter text-sm">{getVal(o,['patente','posto'])} {getVal(o,['nome'])}</td>
                        <td className="p-4">
                           <div className="flex flex-wrap gap-1">
                             {expedientes.length > 0 ? expedientes.map((ex, idx) => (
                               <span key={idx} className="bg-blue-50 text-blue-600 text-[8px] font-black uppercase px-2 py-0.5 rounded-md border border-blue-100">{ex.trim()}</span>
                             )) : <span className="text-slate-300">-</span>}
                           </div>
                        </td>
                        <td className="p-4"><span className={`text-[10px] font-black uppercase ${getVal(o,['servico']) === 'UTI' ? 'text-purple-600' : 'text-blue-600'}`}>{getVal(o,['servico']) || '-'}</span></td>
                        <td className={`p-4 text-center text-[11px] ${idade.y >= 45 ? 'text-red-600 bg-red-50 rounded-xl' : 'text-slate-600'}`}>{idade.y}a {idade.m}m {idade.d}d</td>
                        <td className={`p-4 text-center text-[11px] ${servico.y >= 7 ? 'text-red-600 bg-red-50 rounded-xl' : 'text-slate-600'}`}>{servico.y}a {servico.m}m {servico.d}d</td>
                        <td className="p-4 text-right">
                          <div className="flex gap-2 justify-end opacity-0 group-hover:opacity-100 transition-all">
                            <button onClick={() => { 
                               const expArr = getVal(o, ['expediente']).split(',').map(x => x.trim()).filter(x => x !== "");
                            ...
                            }} className="p-2.5 bg-slate-100 text-slate-600 rounded-2xl hover:bg-blue-600 hover:text-white transition-all"><Edit3 size={16}/></button>
                            <button onClick={() => { if(window.confirm(`Apagar ${getVal(o,['nome'])}?`)) sendData('deleteOfficer', { nome: getVal(o,['nome']) }); }} className="p-2.5 bg-slate-100 text-slate-600 rounded-2xl hover:bg-red-600 hover:text-white transition-all"><Trash2 size={16}/></button>
                          </div>
                        </td>
                      </tr>
                    )})}
      ...
      case 'atestados':
         return (
            <div className="bg-white rounded-[3rem] shadow-sm border p-10 animate-fadeIn">
               <div className="overflow-x-auto"><table className="w-full text-left font-bold uppercase"><thead className="text-[10px] text-slate-400 tracking-widest border-b"><tr><th className="p-6">Militar</th><th className="p-6 text-center">Dias</th><th className="p-6">Data</th><th className="p-6">Status</th><th className="p-6 text-right">Ação</th></tr></thead>
                  <tbody className="divide-y divide-slate-100">
                    {data.atestados.map((a, i) => (
                      <tr key={i} className="hover:bg-slate-50 transition-all">
                        <td className="p-6 text-slate-800">{getVal(a,['militar'])}</td>
                        <td className="p-6 text-center text-slate-500">{getVal(a,['dias'])}d</td>
                        <td className="p-6 text-[11px] font-mono text-slate-400">{formatDate(getVal(a,['inicio']))}</td>
                        <td className="p-6"><span className={`px-4 py-1.5 rounded-full text-[9px] font-black ${getVal(a,['status']) === 'Homologado' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>{getVal(a,['status'])}</span></td>
                        <td className="p-6 text-right">{getVal(a,['status']) === 'Pendente' && <button onClick={()=>sendData('updateStatus',{sheet:'Atestados',id:getVal(a,['id']),status:'Homologado'})} className="bg-blue-600 text-white px-4 py-1.5 rounded-xl text-[9px]">Homologar</button>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table></div>
      ...
      <aside className={`${sidebarOpen ? 'w-80' : 'w-24'} bg-slate-950 text-white transition-all duration-500 flex flex-col z-20 shadow-2xl border-r border-white/5`}>
         ...
         <nav className="flex-1 py-10 px-6 space-y-4 overflow-y-auto">
            {[ { id: 'dashboard', label: 'Início', icon: LayoutDashboard }, { id: 'atestados', label: 'Atestados', icon: ShieldAlert, badge: data.atestados.filter(x=>getVal(x,['status'])==='Pendente').length }, { id: 'efetivo', label: 'Efetivo Oficiais', icon: Users } ].map(item => (
              <button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center gap-5 p-5 rounded-[1.5rem] transition-all relative ${activeTab === item.id ? 'bg-blue-600 text-white shadow-2xl shadow-blue-600/40' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}>
                 <div className="relative"><item.icon size={22}/>{item.badge > 0 && <span className="absolute -top-3 -right-3 w-6 h-6 bg-red-500 rounded-full text-[10px] flex items-center justify-center text-white border-4 border-slate-950 font-black">{item.badge}</span>}</div>{sidebarOpen && <span className="text-[10px] font-black uppercase tracking-widest">{item.label}</span>}</button>
            ))}
         </nav>
         <div className="p-8 border-t border-white/5"><button onClick={onLogout} className="flex items-center gap-5 text-slate-600 hover:text-red-400 font-black text-[10px] uppercase tracking-widest w-full p-5"><LogOut size={22}/> {sidebarOpen && 'Sair'}</button></div>
      </aside>
           ...
           <Modal title={formOfficer.nome ? "Editar Militar" : "Incluir Militar"} onClose={() => setShowOfficerModal(false)}>
              <form onSubmit={handleSaveOfficer} className="space-y-6">
                 <div className="grid grid-cols-2 gap-5">
                    <div className="col-span-2"><label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">Nome de Guerra</label><input type="text" required className="w-full p-5 rounded-3xl bg-slate-50 border-0 font-black text-slate-800 mt-1" value={formOfficer.nome || ''} onChange={e => setFormOfficer({...formOfficer, nome: e.target.value})}/></div>
                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">Patente</label><input type="text" required className="w-full p-5 rounded-3xl bg-slate-50 border-0 font-black text-slate-800 mt-1" value={formOfficer.patente || ''} onChange={e => setFormOfficer({...formOfficer, patente: e.target.value})}/></div>
                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">Antiguidade</label><input type="number" required className="w-full p-5 rounded-3xl bg-slate-50 border-0 font-black text-slate-800 mt-1" value={formOfficer.antiguidade || ''} onChange={e => setFormOfficer({...formOfficer, antiguidade: e.target.value})}/></div>
                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">Nascimento</label><input type="date" required className="w-full p-5 rounded-3xl bg-slate-50 border-0 font-black text-slate-800 mt-1" value={formOfficer.nascimento || ''} onChange={e => setFormOfficer({...formOfficer, nascimento: e.target.value})}/></div>
                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 tracking-widest">Data Praça</label><input type="date" required className="w-full p-5 rounded-3xl bg-slate-50 border-0 font-black text-slate-800 mt-1" value={formOfficer.ingresso || ''} onChange={e => setFormOfficer({...formOfficer, ingresso: e.target.value})}/></div>
                    
                    <div className="col-span-2 pt-4 border-t"><label className="text-[10px] font-black uppercase text-blue-500 ml-2 tracking-[0.2em] mb-3 block">Alocação Expediente (Selecione quantos desejar)</label>
                      <div className="grid grid-cols-3 gap-2">
                        {LOCAIS_EXPEDIENTE.map(local => (
                          <button key={local} type="button" onClick={() => handleToggleExpediente(local)} className={`p-3 rounded-2xl text-[10px] font-black transition-all border ${ (formOfficer.expediente || []).includes(local) ? 'bg-blue-600 text-white border-blue-600 shadow-lg' : 'bg-slate-50 text-slate-400 border-slate-100 hover:border-blue-200' }`}>
                             {local}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="col-span-2 pt-4"><label className="text-[10px] font-black uppercase text-indigo-500 ml-2 tracking-[0.2em] mb-3 block">Alocação Serviço (Escolha um)</label>
                 ...
                 <button type="submit" disabled={isSaving} className="w-full py-6 bg-blue-600 text-white font-black rounded-3xl shadow-xl uppercase text-[10px] tracking-[0.25em] active:scale-95 transition-all mt-4">{isSaving ? "A Processar..." : "Salvar no Banco de Dados"}</button>
              </form>
           </Modal>
         )}
      </main>
...
const LoginScreenContainer = ({ onLogin, officersList, isLoading }) => {
  return <LoginScreen onLogin={onLogin} officersList={officersList} isLoading={isLoading} />;
};
