...
const Modal = ({ title, onClose, children }) => (
  <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 font-sans">
    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden animate-fadeIn border border-slate-200">
      <div className="p-5 border-b flex justify-between items-center bg-slate-50">
        <h3 className="font-black text-slate-800 uppercase tracking-tighter">{title}</h3>
        <button onClick={onClose} className="p-2 hover:bg-slate-200 rounded-full transition-all"><CloseIcon size={18}/></button>
      </div>
      ...
      <main className="flex-1 p-4 max-w-lg mx-auto w-full space-y-5">
        ...
        <div className="pt-4"><h3 className="font-black text-slate-800 text-xs uppercase tracking-widest mb-3 flex justify-between items-center">Meus Registros <button onClick={fetchData} className="p-2 bg-white border border-slate-200 rounded-lg shadow-sm active:scale-90"><RefreshCw size={12} className={loading?'animate-spin':''}/></button></h3>
          <div className="space-y-2">
              ...
              <div key={i} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex justify-between items-center">
                <div className="text-xs">
                  ...
                  <div className="flex gap-2 font-bold text-slate-400 text-[8px] uppercase tracking-widest">
                    <span>{formatDate(getVal(item,['inicio', 'data', 'sai', 'datasai']))}</span>
                    {getVal(item,['substituto']) && <span><ArrowRightLeft size={8} className="inline mx-1"/>{formatDate(getVal(item,['entra', 'dataentra']))}</span>}
                  </div>
                </div>
            ...
            {(data.permutas.length === 0 && data.atestados.length === 0) && <p className="text-center text-[10px] text-slate-400 font-bold py-6 uppercase">Sem registos recentes</p>}
          </div>
        </div>
      </main>
...
const MainSystem = ({ user, role, onLogout, globalOfficers, refreshGlobal }) => {
  ...
  const sendData = async (action, payload) => {
    ...
    } catch (e) { setIsSaving(false); alert("Erro ao gravar. Verifique a internet."); }
  };
  ...
  const renderContent = () => {
    if (loading) return <div className="p-20 flex flex-col items-center justify-center gap-4 text-slate-400"><Loader2 className="animate-spin text-blue-600" size={32}/> <p className="font-black text-[10px] uppercase tracking-widest">A Sincronizar com Servidor...</p></div>;
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
                ...
                <div className="md:col-span-4 bg-slate-900 rounded-3xl p-6 md:p-8 text-white shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center border border-slate-800 relative overflow-hidden gap-6">
                   ...
                   <div className="flex items-center gap-4 relative z-10">
                      <div className="bg-blue-600 p-3 md:p-4 rounded-2xl shadow-lg"><Activity size={24}/></div>
                      <div><h3 className="text-lg md:text-xl font-black uppercase tracking-tighter">Status UPI</h3><p className="text-slate-400 text-[9px] font-black uppercase tracking-widest mt-1">Ref: {data.upi.dataReferencia}</p></div>
                   </div>
                   <div className="flex gap-6 md:gap-10 text-center relative z-10 font-black w-full md:w-auto justify-between md:justify-end">
                      <div><p className="text-slate-500 text-[9px] uppercase tracking-widest mb-1">Ocupação</p><p className="text-2xl md:text-3xl">{data.upi.leitosOcupados}</p></div>
                      <div><p className="text-slate-500 text-[9px] uppercase tracking-widest mb-1">Braden</p><p className="text-2xl md:text-3xl text-yellow-500">{data.upi.mediaBraden.toFixed(1)}</p></div>
                      <div><p className="text-slate-500 text-[9px] uppercase tracking-widest mb-1">Fugulin</p><p className="text-2xl md:text-3xl text-green-500">{data.upi.mediaFugulin.toFixed(1)}</p></div>
                   </div>
                </div>

                {/* KPIS MENORES */}
                <div className="bg-white p-6 rounded-3xl border border-slate-200 flex flex-col items-center justify-center group shadow-sm">
