    ...
    const fetchSheetData = async () => {
        ...
        try {
            ...
            if (data && Array.isArray(data)) {
                const normalizedData = data.map(normalizeReport).filter(r => r && r.sectorId && !isNaN(r.timestamp));
    ...
    return (
        <div className="relative w-full h-full min-h-[85vh] bg-slate-50 rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-200 flex flex-col font-sans">
            ...
            <main className="flex-1 overflow-y-auto p-4 lg:p-8 pb-32">
                {view === 'dashboard' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fadeIn">
                        {SECTORS.map(s => {
                            const latest = reports.find(r => r.sectorId === s.id);
                            const shiftInfo = latest ? SHIFTS_PASS.find(sh => sh.id.toLowerCase() === latest.shift.toLowerCase()) : null;
                ...
                {view === 'form' && (() => {
                    const currentSector = SECTORS.find(s => s.id === formData.sectorId);
                    return (
                        ...
                        {Object.keys(filteredAndGrouped).length === 0 ? (
                            ...
                            <div key={date} className="space-y-6">
                                ...
                                {filteredAndGrouped[date].map((r, i) => {
                                    const shiftInfo = SHIFTS_PASS.find(s => s.id.toLowerCase() === String(r.shift).toLowerCase());
                                        ...
                                        <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm transition-all hover:shadow-lg">
                                            <div className="flex justify-between items-start mb-4">
                                                <div className="flex gap-3">
                                                    ...
                                                    <span className={`text-[11px] font-black px-4 py-1.5 rounded-xl ${shiftInfo?.color || 'bg-slate-100 text-slate-500'}`}>{r.shift}</span>
                                                </div>
            ...
            <nav className="absolute bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-100 p-5 flex justify-around items-center rounded-t-[3.5rem] shadow-[0_-15px_40px_rgba(0,0,0,0.05)] z-30 shrink-0">
                ...
                <button onClick={() => { setFormData({...formData, sectorId: ''}); setView('form'); }} className="bg-emerald-700 text-white p-4 rounded-[2rem] -mt-16 shadow-2xl shadow-emerald-500/50 border-[6px] border-white active:scale-90"><Plus size={32}/></button>
